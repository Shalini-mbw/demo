...
// 👤 Step 1: Detect role and setup query accordingly
if ($JWT_userRole == 'admin') {
    // Admin: Fetch all records
    $sql = "SELECT * FROM admin_submit_project_client ORDER BY id ASC";
    $stmt = $conn->prepare($sql);
} elseif ($JWT_userRole == 'client') {
    // Client: Fetch cuid of logged-in user from employee table
    $currentCuid = '';
    $userQuery = "SELECT cuid FROM employee WHERE id = ?";
    $userStmt = $conn->prepare($userQuery);
    $userStmt->bind_param("s", $JWT_userID);
    $userStmt->execute();
    $userResult = $userStmt->get_result();
    if ($userRow = $userResult->fetch_assoc()) {
        $currentCuid = $userRow['cuid'];
    }

    if (!empty($currentCuid)) {
        // Fetch only client-related records
        $sql = "SELECT * FROM admin_submit_project_client WHERE cuid = ? ORDER BY id ASC";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("s", $currentCuid);
    } else {
        // If cuid not found, skip data fetching
        $stmt = null;
    }
}

// 🔄 Step 2: Execute and collect data
if ($stmt) {
    $stmt->execute();
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            ...
            // 📂 Fetch related files
            $files = [];
            $fileQuery = "SELECT id, file_name, file_path, user_id, uploaded_at 
                          FROM admin_submit_project_files 
                          WHERE cuid = ?";
            $fileStmt = $conn->prepare($fileQuery);
            $fileStmt->bind_param("s", $cuid);
            $fileStmt->execute();
...
$file = 'assets/json/final_AddJobs.json';

if (file_put_contents($file, $json)) {
    // echo "JSON written successfully.";
} else {
...
<script src="assets/js/final_addjobs.js"></script>
...
$(document).on('click', '.update_admin_jobs', function(e) {
    ...
    $.ajax({
        ...
        data: {
            job_id: jobId,
            action: "Fetch_job_Othes_details13"
        },
        dataType: "json",
        success: function(response) {
            if (response.status === "success") {
